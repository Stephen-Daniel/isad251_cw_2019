...
class orders extends DbTable
{
	...
	public function __construct()
	{
		...
		// order_id
		$this->order_id = new DbField('orders', 'orders', 'x_order_id', 'order_id', '`order_id`', '`order_id`', 3, 11, -1, FALSE, '`order_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// order_date
		$this->order_date = new DbField('orders', 'orders', 'x_order_date', 'order_date', '`order_date`', CastDateFieldForLike("`order_date`", 0, "DB"), 135, 19, 0, FALSE, '`order_date`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// order_name
		$this->order_name = new DbField('orders', 'orders', 'x_order_name', 'order_name', '`order_name`', '`order_name`', 200, 255, -1, FALSE, '`order_name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->order_name->Nullable = FALSE; // NOT NULL field
		$this->order_name->Required = TRUE; // Required field
		$this->order_name->Sortable = TRUE; // Allow sort
		$this->fields['order_name'] = &$this->order_name;

		// order_email
		$this->order_email = new DbField('orders', 'orders', 'x_order_email', 'order_email', '`order_email`', '`order_email`', 200, 255, -1, FALSE, '`order_email`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->order_email->Nullable = FALSE; // NOT NULL field
		$this->order_email->Required = TRUE; // Required field
		$this->order_email->Sortable = TRUE; // Allow sort
		$this->fields['order_email'] = &$this->order_email;
	}
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->order_name->DbValue = $row['order_name'];
		$this->order_email->DbValue = $row['order_email'];
	}
	...
	public function getViewUrl($parm = "")
	{
		if ($parm != "")
			...
			$url = $this->keyUrl("ordersview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	public function keyUrl($url, $parm = "")
	{
		...
		if ($this->order_id->CurrentValue != NULL) {
			$url .= "order_id=" . urlencode($this->order_id->CurrentValue);
		} else {
	...
	public function getRecordKeys()
	{
		...
		if (Param("key_m") !== NULL) {
			...
			if (Param("order_id") !== NULL)
				$arKeys[] = Param("order_id");
	...
	public function loadListRowValues(&$rs)
	{
		...
		$this->order_name->setDbValue($rs->fields('order_name'));
		$this->order_email->setDbValue($rs->fields('order_email'));
	}
	...
	public function renderListRow()
	{
		...
		// order_name
		$this->order_name->ViewValue = $this->order_name->CurrentValue;
		$this->order_name->ViewCustomAttributes = "";

		// order_email
		$this->order_email->ViewValue = $this->order_email->CurrentValue;
		$this->order_email->ViewCustomAttributes = "";
		...
		// order_name
		$this->order_name->LinkCustomAttributes = "";
		$this->order_name->HrefValue = "";
		$this->order_name->TooltipValue = "";

		// order_email
		$this->order_email->LinkCustomAttributes = "";
		$this->order_email->HrefValue = "";
		$this->order_email->TooltipValue = "";
	...
	public function renderEditRow()
	{
		...
		// order_name
		$this->order_name->EditAttrs["class"] = "form-control";
		$this->order_name->EditCustomAttributes = "";
		if (!$this->order_name->Raw)
			$this->order_name->CurrentValue = HtmlDecode($this->order_name->CurrentValue);
		$this->order_name->EditValue = $this->order_name->CurrentValue;
		$this->order_name->PlaceHolder = RemoveHtml($this->order_name->caption());

		// order_email
		$this->order_email->EditAttrs["class"] = "form-control";
		$this->order_email->EditCustomAttributes = "";
		if (!$this->order_email->Raw)
			$this->order_email->CurrentValue = HtmlDecode($this->order_email->CurrentValue);
		$this->order_email->EditValue = $this->order_email->CurrentValue;
		$this->order_email->PlaceHolder = RemoveHtml($this->order_email->caption());
		...
		if (!$doc->ExportCustom) {
			...
			if ($doc->Horizontal) { // Horizontal format, write header
				...
				if ($exportPageType == "view") {
					...
					$doc->exportCaption($this->order_name);
					$doc->exportCaption($this->order_email);
				} else {
					$doc->exportCaption($this->order_id);
					$doc->exportCaption($this->order_date);
					$doc->exportCaption($this->order_name);
			...
			if ($recCnt >= $startRec) {
				...
				if (!$doc->ExportCustom) {
					...
					if ($exportPageType == "view") {
						...
						$doc->exportField($this->order_name);
						$doc->exportField($this->order_email);
					} else {
						$doc->exportField($this->order_id);
						$doc->exportField($this->order_date);
						$doc->exportField($this->order_name);
