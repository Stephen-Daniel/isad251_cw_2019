...
/**
 * Table class for orders_items
 */
class orders_items extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'orders_items';
		$this->TableName = 'orders_items';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`orders_items`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// order_id
		$this->order_id = new DbField('orders_items', 'orders_items', 'x_order_id', 'order_id', '`order_id`', '`order_id`', 3, 11, -1, FALSE, '`order_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// product_id
		$this->product_id = new DbField('orders_items', 'orders_items', 'x_product_id', 'product_id', '`product_id`', '`product_id`', 3, 11, -1, FALSE, '`product_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// quantity
		$this->quantity = new DbField('orders_items', 'orders_items', 'x_quantity', 'quantity', '`quantity`', '`quantity`', 3, 11, -1, FALSE, '`quantity`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`orders_items`";
	}
	...
	{
		return "`order_id` = @order_id@ AND `product_id` = @product_id@";
	}
	...
	{
		global $Language;
		if ($pageName == "orders_itemsview.php")
			return $Language->phrase("View");
		elseif ($pageName == "orders_itemsedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "orders_itemsadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("orders_itemsview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("orders_itemsview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "orders_itemsadd.php?" . $this->getUrlParm($parm);
		else
			$url = "orders_itemsadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("orders_itemsedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("orders_itemsadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("orders_itemsdelete.php", $this->getUrlParm());
	}
	...
	public function getFilterFromRecordKeys($setCurrent = TRUE)
	{
		...
		foreach ($arKeys as $key) {
			...
			if ($setCurrent)
				$this->order_id->CurrentValue = $key[0];
			else
				$this->order_id->OldValue = $key[0];
			if ($setCurrent)
				$this->product_id->CurrentValue = $key[1];
			else
				$this->product_id->OldValue = $key[1];
